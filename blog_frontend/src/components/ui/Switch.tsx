...
const Switch: React.FC<SwitchProps> = ({ checked, onChange, label }) => (
  <label className="flex items-center cursor-pointer select-none">
    <div className="relative">
      ...
      <div className={`dot absolute left-1 top-1 w-4 h-4 rounded-full bg-bg-input transition-transform duration-200 dark:bg-text-light ${checked ? 'translate-x-4 border-2 border-primary dark:border-primary' : ''}`}></div>
    </div>
